...
Trước hết tạo `src/lib/rsvp/types.ts` với đúng một kiểu — Task 7 sẽ bổ sung phần còn lại vào cùng file này:
...
Rồi tạo `src/components/sections/types.ts`:
...
Bổ sung vào `src/lib/rsvp/types.ts` (file đã có `LoiChuc` từ Task 4 — giữ nguyên kiểu đó):
...
### Task 13: Trang thiệp công khai theo trạng thái

**Files:**
- Create: `src/app/[slug]/page.tsx`
- Create: `src/components/ThongBaoTrangThai.tsx`
- Modify: `src/app/layout.tsx`
- Test: `src/components/__tests__/ThongBaoTrangThai.test.tsx`

**Interfaces:**
- Consumes: `layThiepTheoSlug`, `layLoiChucDaDuyet`, `layTheme`, `tinhTrangThai`, `InvitationRenderer`
- Produces: `<ThongBaoTrangThai trangThai={'nhap' | 'het-han'} />`

- [ ] **Step 1: Viết test (sẽ fail)**

Tạo `src/components/__tests__/ThongBaoTrangThai.test.tsx`:
...
describe('ThongBaoTrangThai', () => {
  it('thiệp chưa xuất bản báo chưa mở, không lộ thông tin cô dâu chú rể', () => {
...
Run: `npm test -- ThongBaoTrangThai`
Expected: FAIL.

- [ ] **Step 3: Cài đặt**

Tạo `src/components/ThongBaoTrangThai.tsx`:
...
- [ ] **Step 5: Viết trang thiệp công khai**

Tạo `src/app/[slug]/page.tsx`:
...
- [ ] **Step 6: Đặt viewport và cho phép ảnh từ Supabase**
...
```bash
git add -A
git commit -m "feat: thêm trang thiệp công khai rẽ nhánh theo trạng thái vòng đời"
```

---

### Task 14: Kiểm thử đầu-cuối

**Files:**
- Create: `playwright.config.ts`
- Create: `e2e/quan-tri.spec.ts`, `e2e/thiep.spec.ts`
- Create: `docs/kiem-tra-tay.md`

**Interfaces:**
- Consumes: toàn bộ ứng dụng
- Produces: lệnh `npm run e2e`

- [ ] **Step 1: Cài Playwright và cấu hình**
...
Thêm vào `package.json`, mục `scripts`: `"e2e": "playwright test"`.
...
Tạo một tài khoản nhân viên dùng riêng cho kiểm thử trong Supabase và điền vào `.env.local`.

- [ ] **Step 2: Viết test luồng quản trị**

Tạo `e2e/quan-tri.spec.ts`:
...
test('tạo đám cưới, chọn phần hiển thị, xuất bản rồi khách xem được', async ({ page }) => {
  await dangNhap(page)
  ...
  await expect(trangKhach.locator('[data-section="su-kien"]')).toHaveCount(1)

  await khach.close()
})
```

- [ ] **Step 3: Viết test luồng khách mời**

Trước khi chạy, tạo sẵn trong Supabase một thiệp `slug = 'e2e-co-dinh'` đã xuất bản, hạn còn dài, `noi_dung` là JSON của `thiepMau` với `slug` đổi thành `e2e-co-dinh`.

Tạo `e2e/thiep.spec.ts`:
...
test('gửi xác nhận tham dự thành công', async ({ page }) => {
  ...
  await page.getByLabel('Đến ăn ngày').selectOption({ index: 1 })
...
Run: `npm run e2e`
Expected: PASS, 14 test (7 test × 2 thiết bị).

- [ ] **Step 5: Viết danh sách kiểm tra tay**

Tạo `docs/kiem-tra-tay.md`:
...
```bash
git add -A
git commit -m "test: thêm kiểm thử đầu-cuối và danh sách kiểm tra tay"
```

---

## Sau khi hoàn thành v1

Kiểm chứng ranh giới với session thiết kế: thêm một theme mới vào `src/lib/themes/` và thay ruột một section trong `src/components/sections/`. Nếu làm được mà **không sửa dòng nào** ngoài hai thư mục đó cộng `src/lib/motifs/`, hợp đồng đã đúng. Nếu phải sửa renderer, trang quản trị hay lớp dữ liệu, dừng lại và sửa hợp đồng trước khi nhân bản 30 bộ — vì sai sót đó sẽ nhân lên 30 lần.
